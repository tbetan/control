...
void loop()
{
    ...
    if (tiempoActual - tiempoAnterior >= TsMs)
    {
        ...
        // 4. Limitar a rango PWM (anti-windup por recorte)
        salidaControl = constrain(salidaControl, 0, 255);

        // 5. Aplicar al motor
        analogWrite(ENA, (int)salidaControl);

        // 6. Actualizar historial
        salidaControlAnterior = salidaControl;
        ...
        Serial.println((int)salidaControl);
    }
}
