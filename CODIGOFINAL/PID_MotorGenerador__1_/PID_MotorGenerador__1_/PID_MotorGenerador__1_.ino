...
//
//  Envia:
//    D,tiempo,voltaje,error,pwm,u_f,u_sat  -> una linea por muestra
//       u_f   = salida del controlador antes de saturar  (voltios)
//       u_sat = lo que realmente recibe el motor          (voltios)
...
void loop()
{
    ...
    if (tiempoActual - tiempoAnterior >= TsMs)
    {
        ...
        // 4. Sumar el punto de operacion  ->  U(s)_f  (antes de saturar)
        float u_f = salidaControl + BIAS;

        // 5. Saturar al rango del actuador  ->  U(s)_sat
        float pwm = u_f * PWM_POR_VOLT;
        pwm = constrain(pwm, 0.0, 255.0);
        pwmAplicado = (int)pwm;
        float u_sat = pwm / PWM_POR_VOLT;

        // 6. Aplicar al motor
        analogWrite(ENA, pwmAplicado);

        // 7. Anti-windup: el integrador recuerda lo que REALMENTE se aplico
        salidaControlAnterior = u_sat - BIAS;
        ...
        // 9. Enviar muestra:  t, voltaje, error, pwm, U(s)_f, U(s)_sat
        Serial.print("D,");
        ...
        Serial.print(pwmAplicado);                                Serial.print(",");
        Serial.print(u_f, 3);                                     Serial.print(",");
        Serial.println(u_sat, 3);
    }
}
